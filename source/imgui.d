...
extern(C) nothrow void ImGuiRenderDrawLists(ImDrawList** cmd_lists, int count)
{
	if (count == 0)
		return;
...
void ImGuiInit(GLFWwindow* window, bool install_callbacks)
{
	g_window = window;
    ...
    {
        glfwSetMouseButtonCallback(window, &ImGuiMouseButtonCallback);
        glfwSetScrollCallback(window, &ImGuiScrollCallback);
        glfwSetKeyCallback(window, &ImGuiKeyCallback);
        glfwSetCharCallback(window, &ImGuiCharCallback);
    }
}
...
extern(C) nothrow const(char)* ImGuiGetClipboardText()
{
    return glfwGetClipboardString(g_window);
}

extern(C) nothrow void ImGuiSetClipboardText(const(char)* text)
{
    glfwSetClipboardString(g_window, text);
}

extern(C) nothrow void ImGuiMouseButtonCallback(GLFWwindow*, int button, int action, int /*mods*/)
{
    if (action == GLFW_PRESS && button >= 0 && button < 3)
        g_MousePressed[button] = true;
}

extern(C) nothrow void ImGuiScrollCallback(GLFWwindow*, double /*xoffset*/, double yoffset)
{
    g_MouseWheel += cast(float)yoffset; // Use fractional mouse wheel, 1.0 unit 5 lines.
}

extern(C) nothrow void ImGuiKeyCallback(GLFWwindow*, int key, int, int action, int mods)
{
    auto io = ig_GetIO();
...
extern(C) nothrow void ImGuiCharCallback(GLFWwindow*, uint c)
{
    if (c > 0 && c < 0x10000)
    {
